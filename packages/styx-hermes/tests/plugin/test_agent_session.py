...
def test_set_then_get_session() -> None:
    ...
    _agent_session.set_session("alyona", client)
    out = _agent_session.get_session()
    assert out is not None
    agent_id, got_client = out
    assert agent_id == "alyona"
...
def test_set_replaces_previous() -> None:
    ...
    _agent_session.set_session("alyona", c1)
    _agent_session.set_session("vera", c2)
    out = _agent_session.get_session()
    assert out is not None
    agent_id, client = out
    assert agent_id == "vera"
    assert client is c2
