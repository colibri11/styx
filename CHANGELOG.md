...
## [styx-core 1.7.1] — 2026-09-03

Подготовлено, но не опубликовано: строгий structured-output контракт для
редукции завершённых когнитивных актов.

### Исправлено

- Act-residue reducer передаёт Ollama динамическую JSON Schema, ограниченную
  координатами конкретного акта. Недоступные channel/action/observation
  ссылки нельзя сгенерировать как формально допустимый результат.
- Аффективные и неаффективные residues разведены непересекающимися ветками:
  `affect` обязателен только для `affective_coordinate`, включая запрет
  `affect: null` у остальных ролей; сохраняется не более одной аффективной
  координаты.
- Повторные одинаковые evidence refs сворачиваются детерминированно с
  сохранением первого вхождения: Ollama не гарантирует исполнение schema
  keyword `uniqueItems`, а дубликат не является новым свидетельством.
- Краткий `no_residue.reason` ограничен при генерации 240 символами. Защитный
  приём результатов от legacy/mock clients допускает до 2048 символов и
  детерминированно усекает превышение вместо terminal failure.
- Общий Ollama client сохранил `format: "json"` по умолчанию и получил
  opt-in schema-object для callers, которым нужен constrained decoding.

### Проверка

- Исходный production fixture воспроизводит contract failure без схемы и
  проходит через schema-object и существующий поствалидатор со схемой.
- Fresh Docker core на PostgreSQL + Ollama: **1858 passed / 16 expected
  skipped / 0 failed**; Hermes compatibility: **236 passed / 0 failed**.
...
боевой стек, 2026-07-01) — три независимых фикса observability/robustness
...
- **memory_daily_consolidation: доброкачественная гонка supersede/delete
  ...
  залипло на проде). Консолидировать нечего — это ожидаемый исход гонки, а
...
Редеплой styx-daemon на прод (builder / боевой стек) и рестарт залипших
